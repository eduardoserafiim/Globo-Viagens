...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !validar_csrf($_POST['csrf_token'] ?? null)) {
    header('Location: ../../public_html/destino-criar.php?erro=csrf');
    exit;
}
...
if (($_POST['acao'] ?? '') === 'alterar_status') {
    ...
    header('Location: ../../public_html/destinos.php?status=atualizado');
    exit;
}
...
if (($_POST['acao'] ?? '') === 'editar') {
    ...
    if (!$destinoAtual) {
        header('Location: ../../public_html/destinos.php?erro=destino');
        exit;
    }
    ...
    if ($nome === '' || strlen($estado) !== 2 || $tipo === '' || $resumo === '' || $descricao === '' || $localizacao === '' || !in_array($status, ['ativo', 'em_uso', 'inativo'], true) || $preco === false || $preco < 0) {
        header('Location: ../../public_html/destino-editar.php?id=' . (int) $id . '&erro=validacao');
        exit;
    }
...
if (
    ...
    header('Location: ../../public_html/destino-criar.php?erro=validacao');
    exit;
}
...
header('Location: ../../public_html/destinos.php?sucesso=destino');
exit;