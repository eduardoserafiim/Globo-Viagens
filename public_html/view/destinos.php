                ...
                <div class="admin-destination-list">
                    ...
                    <?php foreach ($destinos as $destino): $status = $destino['status'] ?? ((int) $destino['ativo'] === 1 ? 'ativo' : 'inativo'); ?>
                        <article class="admin-destination-row destination-row-rich">
                            ...
                            <div class="destination-row-actions"><a class="icon-action" href="destino?id=<?= (int) $destino['id'] ?>" title="Visualizar destino" aria-label="Visualizar destino"><i class="fa-solid fa-eye"></i></a><a class="icon-action" href="destino-editar?id=<?= (int) $destino['id'] ?>" title="Editar destino" aria-label="Editar destino"><i class="fa-solid fa-pen"></i></a><form method="post" action="../../private_html/controllers/destinoController.php"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token()) ?>"><input type="hidden" name="acao" value="alterar_status"><input type="hidden" name="id" value="<?= (int) $destino['id'] ?>"><select name="status" aria-label="Alterar status" onchange="this.form.submit()"><option value="ativo" <?= $status === 'ativo' ? 'selected' : '' ?>>Ativo</option><option value="em_uso" <?= $status === 'em_uso' ? 'selected' : '' ?>>Em uso</option><option value="inativo" <?= $status === 'inativo' ? 'selected' : '' ?>>Inativo</select></form></div>
                        </article>
                    <?php endforeach; ?>
                </div>
            </section>
